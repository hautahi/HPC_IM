"""
- This program calls functions to run RIS algorithms in series on a desktop
...
def main():
    ...
    R = get_R(G,p,mc)
    t = time.time() - start
    name = ['./output/','loop1','_ser','_n',nodes,'_mc',mc,'_p',p,'_c',1,'_desk','.csv']
    ...
    if (opt == 'approx') | (opt == 'both'):
        ...
        _ = ris(R,k)
        t = time.time() - start
        name = ['./output/','loop2-RIS','_ser','_n',nodes,'_k',k,'_mc',mc,'_p',p,'_c',1,'_desk','.csv']
        name = ''.join(map(str, name))
        pd.DataFrame({'time': [t]}).to_csv(name, index=False)
    ...
    if (opt == 'exact') | (opt == 'both'):
        ...
        t = time.time() - start
        name = ['./output/','loop2-Exact','_ser','_n',nodes,'_k',k,'_mc',mc,'_p',p,'_c',1,'_desk','.csv']
        name = ''.join(map(str, name))
        pd.DataFrame({'time': [t]}).to_csv(name, index=False)
